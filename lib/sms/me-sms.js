...
var twilio = require('./config.js').Twilio;
...
var getPhoneNumber = function () {
    if (numberIndex + 1 > twilio.Numbers.length) {
        numberIndex = 0;
    }
    return twilio.Numbers[numberIndex++];
};
...
SMS.prototype.sendMessage = function (message, peerCallback) {
    ...
    self.messageValidate(message, OPERATION_SCHEMAS.sendMessage, function(error) {
        if (error) {
            ...
            if (self.sendHistory[messageInfo.sendTo]) {
                if (sendDate.getTime() - self.sendHistory[messageInfo.sendTo] <= sendInterval) {
                    logger.error(210001);
            ...
            if (messageInfo.conditional.messageText) {
                tempMessage.body = messageInfo.conditional.messageText;
            }
            if (messageInfo.conditional.mediaUrl) {
                tempMessage.mediaUrl = messageInfo.conditional.mediaUrl;
            }
            if (messageInfo.statusCallback) {
                tempMessage.statusCallback = messageInfo.statusCallback;
            }
            self.client.messages.create(tempMessage, function (error, message) {
                if (error) {
                    var logError = {
